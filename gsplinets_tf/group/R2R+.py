...
class H:
	...
	e = tf.constant(np.array([1.]), dtype=tf.float32)
	...
	def dist( h_1, h_2):
		...
		dist = ( H.log( H.prod( H.inv(h_1), h_2)))[...,0] # Since each h is a list of length 1 we can do [...,0]
														  # And since the splines are symmetric we do not need tf.abs(...)
		return dist
	...
	def grid( N , dc ): # dc: A list of which the length corresponds to the number of exponential coordinates (dimensionality of H)
		if N==0:
			...
			c_list = dc*tf.constant(np.array([np.linspace(-(N-1)/2, (N-1)/2,N)]).transpose(),dtype=tf.float32)
			h_list = H.exp(c_list)
		return h_list
